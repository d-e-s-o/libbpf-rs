...
enum MapMeta {
    ...
    Datasec {
        mmap_idx: usize,
        read_only: bool,
        /// Name of the type in the generated `types` module describing the
        /// contents of the data section backing this map.
        ///
        /// For the regular data sections this is the same as the map name,
...
/// data.
///
/// This mirrors what `bpftool` does, which keys off the presence of an
/// initial value and not off `BPF_F_MMAPABLE`.
...
fn gen_skel_c_skel_constructor(skel: &mut String, object: &Object, name: &str) -> Result<()> {
    ...
    // Note that we have to emit an entry for every map that `maps` yields
    // and in that very order, because `MapsData` derives the `mmap_idx` it
    // hands to `map_mmap_ptr` from the same (unfiltered) iterator.
